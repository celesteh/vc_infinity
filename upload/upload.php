...
<?php echo "<!-- is_audio {$is_audio} id {$panel} scaled_x {$scaled_x} scaled_y {$scaled_y} scaled_width {$scaled_width} scaled height {$scaled_height} X {$x} Y {$y} panel {$panel} active {$active} submit {$submit} ok {$ok} selected {$selected} upload {$upload} error {$error} file_ype {$file_type} -->"; ?>
<div class="wrapper">
